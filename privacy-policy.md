...
Local Allergies ("we", "us", "our") is developed and operated by **NoahMDev**.

For questions about this Privacy Policy, contact us at: **NMTRADE2018@GMAIL.COM**
...
**Storage:** Account credentials are processed and stored by **Supabase** (our authentication and database provider), hosted in the EU (Ireland, AWS eu-west-1). Passwords are never stored in plain text; Supabase stores only a bcrypt hash.

---

### 4.2 Health and Symptom Data (Special Category)

**What we collect:** Symptom logs you enter yourself, including: symptom type (e.g. sneezing, itchy eyes, congestion — from a fixed set of 6 categories), severity score (1–10), time of day, whether medication was taken, free-text medication name, and a human-readable location label (place name or address) at the time of logging where available. These constitute **health data** under UK/EU GDPR Article 9.
...
**What we collect:** Your in-app preferences including: theme (light/dark), notification schedules and alert thresholds, allergen profile selections, language preference, onboarding completion status, and a record of which one-time milestone notifications have been sent (e.g. "primary trigger identified") to avoid duplicate alerts.
...
### 6.4 Supabase
**Purpose:** Authentication (email/password, Google Sign-In, Apple Sign-In) and account data storage.  
**Data involved:** Email address, hashed password (email sign-in), identity tokens issued by Google or Apple (social sign-in), and account identifiers.  
**Hosting:** EU (Ireland, AWS eu-west-1) — data does not leave the EEA.  
**Privacy policy:** [https://supabase.com/privacy](https://supabase.com/privacy)

### 6.5 Google Sign-In / Apple Sign-In
...
You have the following rights regarding your personal data. To exercise any right, contact us at **NMTRADE2018@GMAIL.COM**.
